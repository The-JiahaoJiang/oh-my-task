...
import { CONFIG_SCHEMA_VERSION, TASK_SCHEMA_VERSION, } from "./types.js";
const TASK_STATUSES = new Set(["planned", "in-progress", "blocked", "completed", "archived"]);
export function parseTaskMetadata(value) {
    ...
    if (project)
        requiredString(project, "name", issues, {}, "project.name");
    ...
    if (object.latestSession !== undefined)
        validateSession(object.latestSession, "latestSession", issues);
    ...
    if (issues.length)
        throw new ValidationError("Task frontmatter is invalid.", issues);
    return object;
}
export function parseConfig(value) {
    ...
    if (issues.length)
        throw new ValidationError("Configuration is invalid.", issues);
    return object;
}
function validateSession(value, path, issues) {
...
function checkVersion(kind, value, supported) {
    if (value !== supported)
        throw new UnsupportedSchemaVersionError(kind, value, supported);
}
...
function requiredString(object, key, issues, constraints = {}, path = key) {
...
function optionalString(object, key, issues) {
...
function requiredInteger(object, key, issues, minimum, path = key) {
    const value = object[key];
    if (!Number.isInteger(value) || value < minimum) {
        issues.push({ path, message: `must be an integer greater than or equal to ${minimum}`, actual: value });
    }
}
function requiredBoolean(object, key, issues) {
    if (typeof object[key] !== "boolean")
        issues.push({ path: key, message: "must be a boolean", actual: object[key] });
}
function requiredDate(object, key, issues, path = key) {
    const value = object[key];
    if (typeof value !== "string" || Number.isNaN(Date.parse(value))) {
        issues.push({ path, message: "must be an ISO-8601 date string", actual: value });
    }
}
function requiredEnum(object, key, allowed, issues) {
