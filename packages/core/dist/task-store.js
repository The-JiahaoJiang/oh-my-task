...
import { IncompletePlanError, StaleRevisionError, TaskAlreadyExistsError, TaskNotFoundError, ValidationError, } from "./errors.js";
import { addSession, appendCheckpoint, countCheckpoints, createTaskBody, hasIncompletePlanItems, updateCurrentState, updatePlanItems, } from "./task-body.js";
import { getOhMyTaskPaths } from "./paths.js";
...
export class TaskStore {
    ...
    async read(id) {
        validateTaskId(id);
        try {
            return parseTaskDocument(await readFile(this.taskPath(id), "utf8"));
        }
        catch (error) {
    ...
    async associate(id, baseRevision, session) {
    ...
    async checkpoint(id, input) {
        if (!input.progress.trim())
            throw new ValidationError("Checkpoint is invalid.", [{ path: "progress", message: "must not be empty" }]);
        if (!input.nextAction.trim())
            throw new ValidationError("Checkpoint is invalid.", [{ path: "nextAction", message: "must not be empty" }]);
        return this.mutate(id, input.baseRevision, (document) => {
            ...
            if (active)
                document.metadata.activePlanItem = active;
            if (input.session) {
    ...
    async complete(id, input) {
        return this.mutate(id, input.baseRevision, (document) => {
            if (hasIncompletePlanItems(document.body) && !input.force)
                throw new IncompletePlanError(id);
    ...
    taskPath(id) { return join(this.paths.tasks, `${id}.md`); }
    lockPath(id) { return join(this.paths.locks, `${id}.lock`); }
}
