...
export function parseFrontmatterYaml(source) {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        if (!line.trim() || line.trimStart().startsWith("#"))
            continue;
        if (/\t/.test(line))
            throw yamlError(index, "tabs are not allowed; use two-space indentation");
        const indent = line.length - line.trimStart().length;
        if (indent % 2 !== 0)
            throw yamlError(index, "indentation must use multiples of two spaces");
        const content = line.slice(indent);
        const separator = content.indexOf(":");
        if (separator <= 0)
            throw yamlError(index, "expected a key followed by ':'");
        const key = content.slice(0, separator).trim();
        if (!/^[A-Za-z][A-Za-z0-9_-]*$/.test(key))
            throw yamlError(index, `invalid key: ${key}`);
        while (stack.length > 1 && indent <= stack.at(-1).indent)
            stack.pop();
        const parent = stack.at(-1);
        if (indent !== parent.indent + 2)
            throw yamlError(index, "unexpected indentation level");
        if (Object.hasOwn(parent.value, key))
            throw yamlError(index, `duplicate key: ${key}`);
        ...
        if (!raw) {
            ...
            parent.value[key] = parseScalar(raw, index);
    }
    return root;
}
export function stringifyFrontmatterYaml(value) {
...
function renderObject(value, indent) {
    ...
    for (const [key, item] of Object.entries(value)) {
        ...
        if (item && typeof item === "object" && !Array.isArray(item)) {
            lines.push(prefix, ...renderObject(item, indent + 2));
        }
        else
...
function parseScalar(raw, line) {
    if (raw.startsWith('"')) {
        try {
            ...
            throw yamlError(line, "invalid double-quoted string");
        }
    }
    if (raw.startsWith("'")) {
        if (!raw.endsWith("'") || raw.length < 2)
            throw yamlError(line, "invalid single-quoted string");
        return raw.slice(1, -1).replace(/''/g, "'");
    }
    if (raw === "{}")
        return {};
    if (/^[\[\]{},&*!|>]/.test(raw))
        throw yamlError(line, "unsupported YAML construct in task frontmatter");
    if (raw === "true")
        return true;
    if (raw === "false")
        return false;
